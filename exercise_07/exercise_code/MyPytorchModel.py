...
class MyPytorchModel(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.model = nn.Sequential(
            nn.Linear(hparams["input_size"], hparams["n_hidden"]),
            nn.BatchNorm1d(hparams["n_hidden"]),
            nn.ReLU(),
            nn.Linear(hparams["n_hidden"], hparams["n_hidden"] // 2),
            nn.BatchNorm1d(hparams["n_hidden"] // 2),
            nn.ReLU(),
            nn.Linear(hparams["n_hidden"] // 2, hparams["num_classes"])
        )
    ...
    def configure_optimizers(self):
        ...
        optim = torch.optim.Adam(
          self.parameters(),    # model parameter
          lr=self.hparams.get("learning_rate", 0.001),       # learning rate
          weight_decay=self.hparams.get("weight_decay", 1e-5)  # weight decay
        )
...
class CIFAR10DataModule(nn.Module):
    ...
    def prepare_data(self, stage=None, CIFAR_ROOT="../datasets/cifar10"):
        ...
        # Define transforms (convert to tensors, normalize)
